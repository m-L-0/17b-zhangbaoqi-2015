...
with tf.Session() as sess:
    ...
    for i in range(fm):
        ...
        img = img / 255
        outlayer = sess.run(tf.argmax(EigEm1['y_conv'], 2), feed_dict={EigEm1['x']: img, EigEm1['y_labels']: labels, EigEm1['keep_prob']: 1.0})
        outStr = ''
        for elem in outlayer:
            outStr += str(labelsDict[elem[0]])
        print(outStr, label, end=" ")
        if outStr.replace(' ','') == str(label[0]):
            correct += 1
            print('[step]:',i,'[正确个数]: %d' % (correct))
        else:
            print('[step]:',i)
    print(correct/fm)
    coord.request_stop()
